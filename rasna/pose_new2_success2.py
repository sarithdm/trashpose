...
# Start video capture
cap = cv2.VideoCapture("C:\\Users\\Rasna\\trashpose\\rasna\\rec_vid.mp4")
cv2.namedWindow("Throw Pose Detection", cv2.WINDOW_NORMAL)
cv2.resizeWindow("Throw Pose Detection", 1280, 720)
while cap.isOpened():
    ret, frame = cap.read()
    if not ret:
        break
    ...
    if results.pose_landmarks:
        ...
        if detect_throw_pose(landmarks) or detect_throw_sequence(landmarks):
            cv2.putText(frame, "Throw Pose Detected!", (50, 65), cv2.FONT_HERSHEY_SIMPLEX, 2, (0,0,255), 3)
